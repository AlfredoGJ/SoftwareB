    ...
    public class TarjetaDBController
    {
        ...
        public void Save(Tarjeta tarjeta)
        {
            using (SQLiteCommand command = new SQLiteCommand(database))
            {
                if (tarjeta.id == 0)
                {
                    command.CommandText = "INSERT INTO tarjetas(clave_jugador, clave_partido, tipo) VALUES( @clave_jugador, @clave_partido, @tipo);";
                ...
                {
                    command.CommandText = "UPDATE tarjetas SET clave_jugador = @clave_jugador, clave_partido = @clave_partido, tipo = @tipo;";
        ...
        public List<Tarjeta> GetAll()
        {
            ...
            using (SQLiteDataAdapter adapter = new SQLiteDataAdapter(command, database))
            {
                ...
                {
                    tarjetas.Add(new Tarjeta(row.Field<int>(0), row.Field<string>(2), row.Field<int>(1) ));
                }
            }
            return tarjetas;
        }
